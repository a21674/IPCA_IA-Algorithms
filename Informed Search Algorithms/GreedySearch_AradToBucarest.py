######### Greedy Search - Arad to Bucarest Problem #########
# Este algoritmo de pesquisa usa uma heuristica para a decisão do melhor 
# caminho que é o uso da distância em linha reta até ao objectivo
...
class VetorOrdenado:    
    ...
    #Irá inserir um objecto do tipo Vertice usando a distancia_objectivo como valor a ordenar 
    def insere_ordenado (self, vertice):        
        ...
        for i in range(self.ultima_posicao + 1):
            posicao = i
            if self.valores[i].distancia_objectivo > vertice.distancia_objectivo: #compara o vertice do vetor com o recebido por parametro
                break
    ...
    def imprime(self):
        if self.ultima_posicao == -1:
            ...
            for i in range(self.ultima_posicao + 1):
                print(i, ' - ', self.valores[i].cidade, ' - ', self.valores[i].distancia_objectivo)
        ...
        if atual == self.objectivo: #testa sempre se o atual é o objectivo, mesmo no primeiro nó
            ...
            for adjacente in atual.cidades_adjacentes:
                if adjacente.vertice.visitado == False:
                    ...
                    vetor_ordenado_adjacentes.insere_ordenado(adjacente.vertice)
                    
            vetor_ordenado_adjacentes.imprime()
            
            if vetor_ordenado_adjacentes.valores[0] != None: #verifica se o vetor esta vazio
                self.iniciar_busca(vetor_ordenado_adjacentes.valores[0]) #chamada recursiva pela cidade que tem menos km's pois o vetor está ordenado por ordem crescente
...
def main():
    ...
    vetor.insere_ordenado(mapa.arad)
    vetor.insere_ordenado(mapa.craiova)
    vetor.insere_ordenado(mapa.bucharest)
    vetor.insere_ordenado(mapa.dobreta)
    
    vetor.imprime()
    
    #print(vetor.valores[0], vetor.valores[0].cidade) #mostra o nome e o endereço de memoria do vertice na pos zero
    
    #inicio da busca
    print('')
    print('Inicio da Busca')
    start_search = GreedySearch(mapa.bucharest) #instanciamos a busca dando a cidade objectivo
